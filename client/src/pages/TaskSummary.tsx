      ...
      <div className="grid gap-5 xl:grid-cols-3">
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-xs text-slate-600">
            ...
            {confirmedCause && confirmedCauseLabel ? (
              <div className="flex min-h-5 items-baseline">
            ...
            {delayReason && !confirmedCause && canEdit ? (
              <Button
                type="button"
                variant="outline"
                size="sm"
...
export default function TaskSummary() {
  ...
  const [confirmedCauses, setConfirmedCauses] = useState<Record<string, CauseAttributionRecord>>({})
  const [causeLabels, setCauseLabels] = useState<Record<string, string>>({})
  ...
  const loadTaskCauseSurface = useCallback(async () => {
    ...
    try {
      const [attributions, taxonomy] = await Promise.all([
        listCauseAttributions({ projectId, subjectType: 'task', status: 'confirmed' }),
        listCauseTaxonomy(),
      ])
      const nextConfirmedCauses: Record<string, CauseAttributionRecord> = {}
      for (const attribution of attributions) {
        if (attribution.subject_id) nextConfirmedCauses[attribution.subject_id] = attribution
      }
      const nextCauseLabels: Record<string, string> = {}
      for (const entry of taxonomy.entries) {
        nextCauseLabels[entry.code] = entry.label
      }
      setConfirmedCauses(nextConfirmedCauses)
      setCauseLabels(nextCauseLabels)
    } catch (error) {
      setConfirmedCauses({})
      setCauseLabels({})
      toast({
  ...
  }, [loadData, loadDurationForecasts, loadTaskCauseSurface])
