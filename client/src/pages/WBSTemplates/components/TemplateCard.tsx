  ...
  <TooltipContent>更多操作</TooltipContent>
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-8 z-20 bg-white border border-slate-100 rounded-xl shadow-lg py-1 w-36 text-sm">
                  <Button variant="ghost"
      ...
      <div className="flex items-center gap-4 mb-3 py-2.5 px-3 bg-slate-50 rounded-lg">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-slate-400">层级深度</p>
        </div>
        <div className="w-px h-8 bg-slate-200" />
        <div className="text-center">
          {refDays ? (
      ...
      <div className="flex items-center justify-between mt-auto" onClick={e => e.stopPropagation()}>
        <span className="text-xs text-slate-400">{template.updated_at ? `更新于 ${formatDate(template.updated_at)}` : ''}</span>
