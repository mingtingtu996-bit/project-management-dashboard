...
describe('shared summary dashboards', () => {
  ...
  it('Dashboard only uses shared project summary', async () => {
    ...
    await waitForText(container, ['64%', '81', '2/5', '2/4', '1/3', '3/6', '证照事项', '验收计划', '施工图纸', '任务总结', '模块分析'])
    ...
    expect(container.textContent).toContain('证照事项')
    expect(container.textContent).toContain('验收计划')
    expect(container.textContent).toContain('施工图纸')
    expect(container.textContent).toContain('任务总结')
    expect(container.textContent).toContain('模块分析')
    expect(dashboardSummarySpy).toHaveBeenCalledWith('project-1')
  })
  ...
  it('CompanyCockpit only uses shared project summaries', async () => {
    ...
    await waitForText(container, ['项目总数', '1', '平均总体进度', '72%', '平均健康度', '88', '专项进展', '证照', '验收', '图纸'])
