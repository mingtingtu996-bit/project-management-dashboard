...
describe('project remaining forecast frontend entrypoints', () => {
  it('uses one shared API client for the project-level remaining duration outlet', () => {
    ...
    expect(source.includes('projectRemainingForecast: DurationMetricDto')).toBe(true)
    ...
    expect(cardSource.includes('projectRemainingForecastDays')).toBe(false)
    expect(cardSource.includes('formatDurationMetric')).toBe(true)
  ...
  it('surfaces the same project-level remaining forecast in Dashboard, monthly plan review, and Gantt acceleration context', () => {
    ...
    const taskTimelineSource = readSource('src/pages/GanttView/TaskTimelineView.tsx')
    const projectInfoSource = readSource('src/pages/ProjectInfoModule/ProjectInfoModule.tsx')
    ...
    expect(ganttSource.includes('target_overshoot_days')).toBe(false)
    expect(ganttSource.includes("wizardTargetFeasibility.overshoot?.availability === 'unavailable'")).toBe(true)
    expect(ganttSource.includes('adjustment.currentDurationDays')).toBe(false)
    expect(taskTimelineSource.includes('formatDurationMetric')).toBe(true)
    expect(taskTimelineSource.includes('currentDurationDays}天')).toBe(false)
    expect(projectInfoSource.includes("targetFeasibility.overshoot?.availability === 'unavailable'")).toBe(true)
  })
})
