...
describe('GanttView source contracts', () => {
  ...
  it('lets users accept target acceleration reschedule drafts through governed task-list commits', () => {
    ...
    expect(source.includes("type: 'set_predecessors'")).toBe(true)
    expect(source.includes('predecessorDependencies')).toBe(true)
    expect(source.includes('commitTaskListOperations(operations)')).toBe(true)
    expect(source.includes('applyCommittedTaskRows(committed.rows)')).toBe(true)
    expect(source.includes('recordScheduleAccelerationRecommendationAdoption(id, proposal, {')).toBe(true)
    expect(source.includes('outcomeRef: `task-list-commit:${id}:${committed.revision ?? Date.now()}:acceleration-reschedule`')).toBe(true)
    expect(source.includes("source: 'gantt_target_acceleration_reschedule_commit'")).toBe(true)
