        ...
        <Table className="min-w-full border-separate border-spacing-2">
          ...
          <TableHeader className="sticky top-0 z-10 bg-white">
            <TableRow className="py-3">
              <TableHead scope="col" className="sticky left-0 z-10 bg-white px-3 py-2 text-left text-xs font-medium text-slate-500">证件 / 事项</TableHead>
              {rows[0]?.cells.map((cell) => (
                <TableHead scope="col" key={cell.work_item_id} className="min-w-28 px-3 py-2 text-xs font-medium text-slate-500">
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.certificate_id} className="py-3 even:bg-slate-50/50 hover:bg-slate-100/60">
                <TableHead scope="row" className="sticky left-0 z-10 bg-white px-3 py-2 text-left">
                  <Button variant="ghost"
                    type="button"
                ...
                {row.cells.map((cell) => {
                  ...
                  return (
                    <TableCell key={`${row.certificate_id}-${cell.work_item_id}`} className="px-3 py-2">
                      <Button variant="ghost"
                        type="button"
                        onClick={() => {
