    ...
    : statusKey === 'processing'
      ...
      : statusKey === 'overdue'
        ...
        : statusKey === 'cancelled'
          ...
          : 'bg-amber-400'
  return (
    <Card className={`bg-white rounded-xl border ${borderColor} shadow-[var(--el-1)] hover:shadow-[var(--el-2)] transition-shadow p-5`}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
            statusKey === 'completed' ? 'bg-emerald-50' :
            statusKey === 'processing' ? 'bg-blue-50' :
            statusKey === 'overdue' ? 'bg-red-50' : 'bg-slate-100'
          }`}
          >
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500 mt-0.5 truncate">{milestone.milestone_type}</p>
          </div>
        </div>
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-slate-500">办理进度</span>
          <span className="text-xs font-medium text-slate-700">{progressPercent}%</span>
        </div>
      ...
      <Button variant="ghost"
        ...
        className="w-full flex items-center justify-between text-xs text-slate-500 hover:text-slate-700 py-1.5 transition-colors"
      >
                ...
                <Fragment key={step}>
                  ...
                  {index < PROCESS_STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-1 mb-4 ${
                      currentStep > index ? (milestone.status === '已完成' ? 'bg-emerald-400' : 'bg-blue-300') : 'bg-slate-200'
                    }`}
                    />
                  )}
                </Fragment>
