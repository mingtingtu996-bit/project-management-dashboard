...
async function upsertSiteShutdownEvents(projectId: string, signals: WeatherImpactSignal[]) {
  const rowsByKey = new Map<string, Record<string, unknown>>()
  for (const signal of signals) {
    const event = signal.siteShutdownEvent
    if (!event) continue
    const source = 'weather_forecast'
    const key = `${projectId}|${event.eventDate}|${event.eventType}|${source}`
    rowsByKey.set(key, {
      ...
      severity: 'red',
      source,
      source_forecast_id: typeof signal.evidence.sourceForecastId === 'string'
        ? signal.evidence.sourceForecastId
        : null,
      source_provider: typeof signal.evidence.provider === 'string' ? signal.evidence.provider : null,
      source_url: typeof signal.evidence.sourceUrl === 'string' ? signal.evidence.sourceUrl : null,
  ...
  const { error } = await (supabase as any)
    .from('site_shutdown_events')
    .upsert(rows, { onConflict: 'project_id,event_date,event_type,source' })

  if (error) throw error
}

function weatherImpactAssetKey(signal: WeatherImpactSignal) {
  return [
...
function weatherImpactCandidatePayload(signal: WeatherImpactSignal) {
  return {
    impactType: signal.impactType,
    climateSignal: signal.climateSignal,
    severity: signal.severity,
    forecastDate: signal.forecastDate,
    multiplier: signal.multiplier,
    confidenceDelta: signal.confidenceDelta,
    actionPolicy: signal.actionPolicy,
    sourceReliability: signal.sourceReliability,
    sourceReliabilityScore: signal.sourceReliabilityScore,
    reason: signal.reason,
    siteShutdownEvent: signal.siteShutdownEvent ?? null,
    evidence: signal.evidence,
    sourceForecastId: typeof signal.evidence.sourceForecastId === 'string'
      ? signal.evidence.sourceForecastId
      : null,
  }
}

async function persistWeatherImpactGovernanceCandidates(projectId: string, signals: WeatherImpactSignal[]) {
  ...
  for (const signal of signals) {
    try {
      await createAndPersistAlgorithmAssetCandidateEvent({
        assetKey: weatherImpactAssetKey(signal),
        sourceSystem: 'weatherForecastImpactService',
        assetType: 'signal',
        companyId,
        projectId,
        candidatePayload: weatherImpactCandidatePayload(signal),
        ...
        evidence: {
          replayPassed: false,
          conflictFree: signal.actionPolicy !== 'candidate_only' ? true : false,
          rollbackTarget: null,
        },
      })
    } catch (error) {
      logger.warn('[weatherForecastImpactService] failed to persist governance candidate event', {
        projectId,
        assetKey: weatherImpactAssetKey(signal),
        error,
      })
...
export async function loadProjectWeatherImpactSignalsWithDiagnostics(input: {
  ...
  const endDate = normalizeText(input.endDate || input.startDate)
  ...
  try {
    ...
    await upsertSiteShutdownEvents(projectId, signals)
    await persistWeatherImpactGovernanceCandidates(projectId, signals)
    return {
      signals,
    ...
    logger.warn('[weatherForecastImpactService] failed to load project weather impact signals', {
      projectId,
      startDate,
      endDate,
      error,
