...
export async function runConstructionOrganizationCandidateAnchorBackfillCli(
  ...
  const report = {
    reportCode: 'construction_organization_candidate_anchor_backfill',
...
if (process.argv[1]?.includes('backfill-construction-organization-candidate-anchors')) {
  runConstructionOrganizationCandidateAnchorBackfillCli()
    .then((report) => {
      process.stdout.write(`${JSON.stringify({
        reportCode: report.reportCode,
        mode: report.result.mode,
        scannedProjectCount: report.result.scannedProjectCount,
        backfillableProjectCount: report.result.backfillableProjectCount,
        backfilledProjectCount: report.result.backfilledProjectCount,
