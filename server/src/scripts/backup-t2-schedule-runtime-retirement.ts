...
function parseArgs(argv: string[]) {
  ...
  if (!output) {
    throw new Error('Usage: backup-t2-schedule-runtime-retirement --output <json-path> [--if-pending]')
  }
  return {
...
async function main() {
  ...
  try {
    ...
    if (applied.rows[0]?.applied === true) {
      if (!options.ifPending) throw new Error('T2_SCHEDULE_RUNTIME_RETIREMENT_ALREADY_APPLIED')
