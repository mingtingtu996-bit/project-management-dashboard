...
function parseArgs(argv: string[]) {
  ...
  if (!output) {
    throw new Error('Usage: backup-progress-knowledge-retirement --output <json-path> [--if-pending]')
  }
  return {
  ...
  try {
    ...
    if (applied.rows[0]?.applied === true) {
      if (!options.ifPending) {
        throw new Error('PROGRESS_KNOWLEDGE_RETIREMENT_ALREADY_APPLIED')
      }
      console.log(JSON.stringify({ status: 'skipped', reason: 'migration_already_applied' }))
      return
    }

    const backup = await captureProgressKnowledgeRetirementBackup(
