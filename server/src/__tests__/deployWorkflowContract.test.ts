...
describe('deploy workflow contract', () => {
  ...
  it('blocks target database mutation until deployment and runtime secrets pass preflight', () => {
    ...
    expect(preflightJob).toContain('ingress_ready_upstream_unavailable')
    expect(preflightJob).toContain('probe_ingress_route()')
    expect(preflightJob).toContain('--resolve "$EXPECTED_PUBLIC_HOST:443:$origin_ip"')
    expect(preflightJob).toContain('--resolve "$EXPECTED_PUBLIC_HOST:80:$origin_ip"')
    expect(preflightJob).toContain('[ "$readyz_status" = 502 ] && [ "$INITIAL_RUNTIME_BOOTSTRAP" = true ]')
    expect(preflightJob).toContain('--max-redirs 0')
    expect(preflightJob).toContain('[ "$redirect_status" = 308 ] || return 1')
    expect(preflightJob).toContain('[ "$redirect_url" = "$DEPLOY_HEALTH_URL" ] || return 1')
    expect(preflightJob).toContain("grep -qi '^Strict-Transport-Security:'")
    expect(preflightJob).toContain('origin_ingress_ready=true')
    expect(preflightJob).toContain('public_domain_ready=true')
    expect(preflightJob).toContain('[ "$DEPLOY_TARGET" = staging ]')
    expect(preflightJob).toContain('publicDomainReady=false')
    expect(preflightJob).toContain('Public-domain preflight failed and origin fallback is not authorized.')
  ...
  it('pins node 22, node24-compatible actions, explicit quality gates, and self-hosted server deployment', () => {
    ...
    expect(deployScript).not.toContain('ALLOW_DIRTY_DEPLOY')
    expect(deployScript).not.toContain('git status --porcelain --untracked-files=no')
    expect(deployScript).toContain('RELEASES_DIR="$APP_DIR/releases"')
    expect(deployScript).toContain('FAILED_RELEASES_DIR="$APP_DIR/failed-releases"')
    expect(deployScript).toContain('STATE_FILE="$APP_DIR/pending-application-release.env"')
    expect(deployScript).toContain('flock -n 9')
    expect(deployScript).toContain('validate_release_archive "$RELEASE_ARCHIVE"')
    expect(deployScript).toContain('tar -xzf "$RELEASE_ARCHIVE" -C "$CANDIDATE_DIR"')
    expect(deployScript).toContain('prepare_runtime_links "$CANDIDATE_DIR"')
    expect(deployScript).toContain('rollback_application_release')
    expect(deployScript).toContain('verify_release_health "$previous_sha"')
    ...
    expect(deployScript).toContain('verify_external_ingress_route()')
    expect(deployScript).toContain('--resolve "$EXPECTED_PUBLIC_HOST:443:$ORIGIN_INGRESS_IP"')
    expect(deployScript).toContain('--resolve "$EXPECTED_PUBLIC_HOST:80:$ORIGIN_INGRESS_IP"')
    ...
    expect(deployScript).toContain('[ "$DEPLOY_TARGET" = staging ]')
    expect(deployScript).toContain('originIngressReady')
    expect(deployScript).toContain('publicDomainReady')
    expect(deployScript).toContain('Public-domain postdeploy verification failed and origin fallback is not authorized.')
  ...
  it('isolates staging and production targets and promotes the tested frontend artifact', () => {
    ...
    const deployScript = readFileSync(resolve(workspaceRoot, 'scripts', 'deploy-lighthouse-server.sh'), 'utf8')
    expect(deployScript).not.toContain('rm -rf client/dist')
    expect(deployScript).not.toContain('tar -xzf "$RELEASE_ARCHIVE" -C "$APP_DIR"')
    expect(deployScript).toContain('tar -xzf "$RELEASE_ARCHIVE" -C "$CANDIDATE_DIR"')
