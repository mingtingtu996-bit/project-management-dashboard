...
describe('check-legacy-object-drop-guard script', () => {
  ...
  it('recognizes the dedicated retirement contracts for migrations 316, 321, and 322', async () => {
    const result = await runLegacyObjectDropGuardCheck([
      '--ci-no-drop-candidates-ok',
      '--scan-migration-drops',
      '--migrations-dir',
      ...
      readMigrationFile: async (path) => {
        if (path.includes('316_task_fact_write_integrity.sql')) return `
          ...
          `
        if (path.includes('321_retire_duplicate_t2_schedule_runtime.sql')) return `
