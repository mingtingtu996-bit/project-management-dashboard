-- Migration 037: Create task_conditions and task_obstacles tables (Final Version)
-- Date: 2026-03-30
-- Problem: Previous execution failed partially, need to clean up and recreate

-- ============================================
-- CLEANUP: Drop existing tables if they exist (to ensure clean state)
-- ============================================
...
-- 创建索引
CREATE INDEX idx_task_conditions_task ON task_conditions(task_id);
CREATE INDEX idx_task_conditions_project ON task_conditions(project_id);
CREATE INDEX idx_task_conditions_type ON task_conditions(condition_type);
CREATE INDEX idx_task_conditions_satisfied ON task_conditions(is_satisfied);
...
-- 创建索引
CREATE INDEX idx_task_obstacles_task ON task_obstacles(task_id);
CREATE INDEX idx_task_obstacles_project ON task_obstacles(project_id);
CREATE INDEX idx_task_obstacles_type ON task_obstacles(obstacle_type);
CREATE INDEX idx_task_obstacles_status ON task_obstacles(status);
CREATE INDEX idx_task_obstacles_severity ON task_obstacles(severity);
