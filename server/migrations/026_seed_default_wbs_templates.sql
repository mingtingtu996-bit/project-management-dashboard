-- Migration 026: Re-seed default WBS templates
-- Date: 2026-03-29
-- Problem: Migration 012's seed data failed because created_by is NOT NULL REFERENCES users(id)
--          The INSERT omitted created_by, causing silent failure.
-- Fix: Re-insert 3 default WBS templates with created_by = NULL (allowed after 012 fixed it)

-- Step 1: Remove any existing seed templates to allow re-insert
DELETE FROM wbs_templates
WHERE template_name IN ('住宅标准WBS模板', '商业综合体WBS模板', '工业厂房WBS模板');

-- Step 2: Insert 住宅标准WBS模板 (住宅)
...
-- Step 5: Verify
SELECT template_name, template_type, is_default FROM wbs_templates
WHERE template_name IN ('住宅标准WBS模板', '商业综合体WBS模板', '工业厂房WBS模板')
