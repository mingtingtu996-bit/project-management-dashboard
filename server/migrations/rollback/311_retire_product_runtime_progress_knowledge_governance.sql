-- Emergency schema rollback for migration 311.
-- After this SQL succeeds, run restore-progress-knowledge-retirement.ts with
-- the matching JSON and SHA-256 files to restore the captured rows.

BEGIN;

CREATE TABLE public.progress_knowledge_sources (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  source_key TEXT NOT NULL UNIQUE,
  source_name TEXT NOT NULL,
  connector_type TEXT NOT NULL CHECK (connector_type IN (
    'external_pdf', 'public_quota', 'enterprise_manual', 'internal_knowledge_base',
    'web_knowledge_base', 'api_connector', 'spreadsheet'
  )),
...
CREATE TABLE public.progress_knowledge_documents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.progress_asset_candidates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  asset_type TEXT NOT NULL CHECK (asset_type IN (
    'duration_seed_candidate', 'process_interleaving_rule', 'wbs_template_candidate',
    'context_correction_factor', 'business_type_schedule_model', 'resource_assumption'
  )),
  ...
  governance_status TEXT NOT NULL DEFAULT 'candidate_only' CHECK (governance_status IN (
    'candidate_only', 'governed_candidate', 'auto_review_package',
    'manual_governance_required', 'rejected'
  )),
  promotion_target TEXT NOT NULL CHECK (promotion_target IN (
    'standard_work_duration_seed', 'process_constraint_rule', 'wbs_reference_days',
    'duration_context_factor', 'business_type_schedule_model', 'construction_organization_assumption'
  )),
...
CREATE TABLE public.progress_asset_calibration_runs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.progress_asset_calibration_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  recommended_action TEXT NOT NULL
    CHECK (recommended_action IN ('auto_canary_ready', 'manual_governance_required', 'candidate_only', 'blocked', 'reject')),
  evidence JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT progress_asset_calibration_results_unique_candidate UNIQUE (run_id, candidate_id)
);

CREATE TABLE public.progress_asset_publication_readiness (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  candidate_id UUID NOT NULL REFERENCES public.progress_asset_candidates(id) ON DELETE CASCADE,
  ...
  readiness_status TEXT NOT NULL CHECK (readiness_status IN (
    'candidate_only', 'auto_review_package', 'auto_canary_ready', 'auto_canary_active',
    'auto_published', 'manual_governance_required', 'blocked', 'runtime_rolled_back'
  )),
  publish_anchor TEXT NOT NULL DEFAULT 'candidate_only' CHECK (publish_anchor IN (
    'candidate_only', 'manual_governance_required', 'trusted_source_auto_publish',
    'guarded_runtime_auto_publish', 'system_curated_publish'
  )),
  ...
  human_review_policy TEXT NOT NULL DEFAULT 'not_publishable' CHECK (human_review_policy IN (
    'zero_human_review_when_gate_passes', 'batch_manual_approval_required', 'not_publishable'
  )),
  release_job_policy TEXT NOT NULL DEFAULT 'retain_candidate_evidence_only' CHECK (release_job_policy IN (
    'enqueue_guarded_canary_release', 'monitor_canary_and_maybe_promote',
    'monitor_published_and_keep_rollback_ready', 'hold_for_governance_batch',
  ...
  observation_window_days INTEGER NOT NULL DEFAULT 0 CHECK (observation_window_days >= 0),
...
CREATE POLICY progress_knowledge_sources_select_admins ON public.progress_knowledge_sources
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));
CREATE POLICY progress_knowledge_documents_select_admins ON public.progress_knowledge_documents
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));
CREATE POLICY progress_asset_candidates_select_admins ON public.progress_asset_candidates
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));
CREATE POLICY progress_asset_calibration_runs_select_admins ON public.progress_asset_calibration_runs
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));
CREATE POLICY progress_asset_calibration_results_select_admins ON public.progress_asset_calibration_results
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));
CREATE POLICY progress_asset_publication_readiness_select_admins ON public.progress_asset_publication_readiness
  FOR SELECT USING (auth.role() = 'service_role' OR EXISTS (
    SELECT 1 FROM public.company_members cm WHERE cm.user_id = auth.uid() AND cm.role = 'company_admin'
  ));

CREATE POLICY progress_knowledge_sources_write_service_role ON public.progress_knowledge_sources
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY progress_knowledge_documents_write_service_role ON public.progress_knowledge_documents
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY progress_asset_candidates_write_service_role ON public.progress_asset_candidates
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY progress_asset_calibration_runs_write_service_role ON public.progress_asset_calibration_runs
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY progress_asset_calibration_results_write_service_role ON public.progress_asset_calibration_results
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY progress_asset_publication_readiness_write_service_role ON public.progress_asset_publication_readiness
  FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');

COMMENT ON TABLE public.progress_asset_candidates IS
  'Emergency-restored candidate-only progress assets. These rows are not product runtime facts.';
COMMENT ON TABLE public.progress_asset_publication_readiness IS
  'Emergency-restored publication readiness retained only for migration 311 rollback.';
