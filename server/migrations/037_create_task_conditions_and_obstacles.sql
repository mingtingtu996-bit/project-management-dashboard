-- Migration 037: Create task_conditions and task_obstacles tables
-- Date: 2026-03-30
...
CREATE POLICY "task_conditions_select_policy" ON task_conditions FOR SELECT
  USING (EXISTS (SELECT 1 FROM tasks t WHERE t.id = task_conditions.task_id));
...
CREATE POLICY "task_obstacles_select_policy" ON task_obstacles FOR SELECT
  USING (EXISTS (SELECT 1 FROM tasks t WHERE t.id = task_obstacles.task_id));
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 037 complete: task_conditions and task_obstacles tables created';
END $$;
