 ...
 *   - /api/jobs scheduler 任务状态接口最低限度可达性
 *   - warning service 产出口径通过 notifications 路由间接验证
 *
 * 用法：
...
async function fetchJSON(url, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      // Non-JSON response is still useful for reachability checks.
    }
...
async function main() {
  ...
  if (!ok || error) {
    console.log(`  失败：HTTP ${status} ${error || ''}`)
    allOk = false
    ...
    if (items.length > 0) {
      ...
      for (let i = 0; i < items.length; i += 1) {
        ...
        if (issues.length > 0) {
          console.log(`  通知[${i}] 结构问题：${issues.join(', ')}`)
          structOk = false
        }
      }
      if (structOk) {
  ...
  if (!uOk || uErr) {
    console.log(`  失败：HTTP ${uStatus} ${uErr || ''}`)
    allOk = false
    ...
    console.log(`  可达，未读计数 ${count ?? '(请检查 data 字段)'}`)
  }

  console.log('\n[3] 定时任务状态 /api/jobs')
  const { ok: jOk, status: jStatus, error: jErr } = await fetchJSON(`${API_URL}/api/jobs`)
  if (jErr) {
    console.log(`  失败：${jErr}`)
    allOk = false
  } else if (jStatus === 404) {
    console.log('  /api/jobs 路由不存在（404），如果项目未暴露该路由可忽略')
  } else if (jStatus === 401 || jStatus === 403) {
    console.log(`  /api/jobs 需要认证（${jStatus}），服务可达；设置 AUTH_TOKEN 可查看完整状态`)
  } else if (jOk) {
    console.log(`  可达，HTTP ${jStatus}`)
  } else {
    console.log(`  HTTP ${jStatus}，可能需要认证或路由行为不同`)
  }

  console.log('\n[4] warning service 产出间接验证')
  console.log('  通过 /api/notifications 路由间接验证，前置步骤已覆盖')
  console.log('  如需深度诊断，请设置 SAMPLE_PROJECT_ID 并确保项目存在活跃风险数据')

  console.log(`\n${allOk ? '全部通过' : '存在问题，请查看上方输出（部分检查需要真实数据才可完整验证）'}`)
