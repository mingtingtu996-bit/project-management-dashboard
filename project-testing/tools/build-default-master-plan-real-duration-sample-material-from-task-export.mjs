...
const DEFAULT_COMPLETED_TASK_EXPORT = path.join(OUTPUT_ROOT, 'source-exports', 'completed-task-export.json')
...
export function parseArgs(argv = process.argv.slice(2)) {
  const options = {
    ...
    completedTaskExport: DEFAULT_COMPLETED_TASK_EXPORT,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === '--completed-task-export') options.completedTaskExport = path.resolve(nextValue())
...
export async function buildDefaultMasterPlanRealDurationSampleMaterialFromTaskExport({
  collectionPackage = DEFAULT_COLLECTION_PACKAGE,
  completedTaskExport = DEFAULT_COMPLETED_TASK_EXPORT,
  output = DEFAULT_OUTPUT,
  sourceName = '',
  ...
  const completedTaskExportPath = path.resolve(completedTaskExport)
  const outputPath = path.resolve(output)
  const collectionPayload = JSON.parse(await readFile(collectionPackagePath, 'utf8'))
  const taskExportPayload = JSON.parse(await readFile(completedTaskExportPath, 'utf8'))
  ...
  const taskRows = readTaskRows(taskExportPayload)
  ...
  for (const row of taskRows) {
    ...
    if (!stableCode || !requestedStableCodes.has(stableCode)) continue
    ...
    const summary = {
      id: taskIdentity(row),
      stableCode,
      title: text(row.title ?? row.name ?? row.taskName ?? row.task_name),
      ...buildTitleDriftDiagnostic(row, request, sampleRequestByTitle),
      blockers,
    }
    if (blockers.length > 0) {
      invalidTasks.push(summary)
      continue
    }
  ...
  const rows = sampleRequests.map((request, index) => {
    ...
    const matchingTasks = validTasksByStableCode.get(stableCode) ?? []
    const selectedTasks = matchingTasks.slice(0, requiredAcceptedSampleCount)
    const missingSampleCount = Math.max(0, requiredAcceptedSampleCount - selectedTasks.length)
    for (const task of selectedTasks) {
      candidateSamples.push(buildMaterialSample({
        task,
        request,
        baselineId,
        projectId,
        collectionPackagePath,
        completedTaskExportPath,
        sourceEvidenceRef: text(evidenceRef),
      }))
    }
    return {
      index: index + 1,
      ...
      matchedCompletedTaskCount: matchingTasks.length,
      selectedCompletedTaskIds: selectedTasks.map(taskIdentity).filter(Boolean),
      missingSampleCount,
      coverageStatus: missingSampleCount === 0 ? 'covered' : 'missing_completed_task_samples',
    }
  })
  const missingStableCodes = rows.filter((row) => row.coverageStatus !== 'covered').map((row) => row.stableCode).filter(Boolean)
  ...
  const requiredFieldBlockers = uniqueText([
  ...
  const material = {
    ...
    completedTaskExportRef: `completed_task_export:${repoRelative(completedTaskExportPath)}#sha256=${await sha256File(completedTaskExportPath)}`,
    sourceEvidence: {
  ...
  const report = {
    schemaVersion: 'workbuddy-default-master-plan-real-duration-sample-material-from-task-export/v1',
    generatedAt: now.toISOString(),
    source: 'build-default-master-plan-real-duration-sample-material-from-task-export',
    status,
    ...
    collectionPackageRef: material.collectionPackageRef,
    completedTaskExportRef: material.completedTaskExportRef,
    summary: {
...
function buildMaterialSample({
  ...
  const actualDurationDays = resolveActualDurationDays(task)
  return {
    id: `real-duration-sample:${taskId}:${stableCode}`,
    stableCode,
    title: text(task.title ?? task.name ?? task.taskName ?? task.task_name ?? request.title),
    projectId,
    taskId,
    actualDurationDays,
    startedAt: text(task.startedAt ?? task.started_at ?? task.actualStart ?? task.actual_start),
    completedAt: text(task.completedAt ?? task.completed_at ?? task.actualFinish ?? task.actual_finish ?? task.actualEnd ?? task.actual_end),
    sourceType: 'completed_task',
    sampleStatus: 'accepted',
    includedInBenchmark: true,
    evidenceRef: text(task.evidenceRef ?? task.evidence_ref) || sourceEvidenceRef,
    candidateReferenceDays: readNumber(request.candidateReferenceDays ?? request.candidate_reference_days),
    requiredAcceptedSampleCount: Math.max(1, readNumber(request.requiredAcceptedSampleCount ?? request.required_accepted_sample_count)),
    metadata: {
      source: 'operator_supplied_completed_task_export',
      materialTemplate: false,
      ...
      completedTaskExportRef: repoRelative(completedTaskExportPath),
...
function completedTaskBlockers(row, { projectId, stableCode, expectedTitle = '' }) {
  ...
  const title = text(row.title ?? row.name ?? row.taskName ?? row.task_name)
  ...
  return uniqueText([
    ...
    actualDurationDays > 0 ? null : 'completed_task_actual_duration_required',
...
function buildTitleDriftDiagnostic(row, request, sampleRequestByTitle) {
  const actualTitle = text(row.title ?? row.name ?? row.taskName ?? row.task_name)
...
function resolveActualDurationDays(row) {
  ...
  const startedAt = parseDate(row.startedAt ?? row.started_at ?? row.actualStart ?? row.actual_start)
  const completedAt = parseDate(row.completedAt ?? row.completed_at ?? row.actualFinish ?? row.actual_finish ?? row.actualEnd ?? row.actual_end)
  if (!startedAt || !completedAt || completedAt < startedAt) return 0
  return Math.floor((completedAt.getTime() - startedAt.getTime()) / 86400000) + 1
}
...
function renderMarkdown(report) {
  const lines = [
    '# Default Master Plan Real Duration Sample Material From Task Export',
    '',
  ...
  if (report.invalidTasks.length > 0) {
    lines.push('', '## Invalid Tasks', '', '| id | stableCode | blockers |', '|---|---|---|')
    for (const task of report.invalidTasks) {
      lines.push(`| ${escapeTable(task.id)} | ${escapeTable(task.stableCode)} | ${escapeTable(task.blockers.join(', '))} |`)
    }
  }
  return `${lines.join('\n')}\n`
}
...
function printHelp() {
  console.log([
    'Usage: node project-testing/tools/build-default-master-plan-real-duration-sample-material-from-task-export.mjs',
    '  [--collection-package <duration-sample-collection-package.json>]',
    '  [--completed-task-export <completed-task-export.json>]',
    '  [--output <real-duration-sample-material.json>]',
    '  --source-name <completed task source name>',
    '  --evidence-ref <operator-reviewed evidence ref>',
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    ...
    const report = await buildDefaultMasterPlanRealDurationSampleMaterialFromTaskExport(options)
