    ...
    scenarios: [
      {
        ...
        priority: 'P0',
        evidenceOwners: ['planning-owner', 'workflow-owner', 'uat-tester'],
        tiers: [
          { name: 'UAT' },
          { name: 'staging' },
          { name: 'live' },
        ...
        evidenceContract: {
          requiredArtifacts: [
            'real-uat-06-plan-state-machine.json',
            'draft-lock-readback.json',
            'approval-audit.json',
          ],
        },
      },
  ...
  return {
    ...
    environmentTargets: {
      ...
      staging: {
        ...
        artifactRoot: `${root}/evidence/staging`,
        writeApprovalRef: 'approval://staging/uat06',
    ...
    scenarios: {
      'REAL-UAT-06': {
        ...
        evidenceOwners: {
          ...
          'workflow-owner': 'owner://workflow',
          'uat-tester': 'owner://uat',
        },
        tiers: {
          staging: {
            targetRefs: {
            ...
            cleanupRef: 'cleanup://uat06',
            approvalRef: 'approval://staging/uat06',
            rollbackRef: 'rollback://staging/uat06',
            monitoringRef: 'monitor://staging/uat06',
          },
        },
      },
    },
...
test('blocks REAL-UAT-06 execution when the selected tier handoff is incomplete', async () => {
  ...
  assert.ok(report.blockers.some((blocker) => blocker.startsWith('scenario:targetRefs.monthlyPlanRef')))
  assert.ok(report.blockers.some((blocker) => blocker.startsWith('scenario:actorRefs.approverRef')))
  assert.ok(report.blockers.some((blocker) => blocker.startsWith('scenario:expectedEvidenceRefs.stateMachineRef')))
  assert.doesNotMatch(written, /password=|postgres:\/\//i)
})

test('blocks REAL-UAT-06 execution when handoff is ready but explicit unlock flags are missing', async () => {
  const { root, matrixFile } = await fixtureRoot()
  const handoffFile = join(root, 'handoff.json')
  const output = join(root, 'report.json')
...
test('blocks REAL-UAT-06 execution when ready handoff refs are not locally resolvable env refs', async () => {
  const { root, matrixFile } = await fixtureRoot()
  const handoffFile = join(root, 'handoff.json')
  const output = join(root, 'report.json')
  await writeJson(handoffFile, readyHandoff(root))
  ...
  assert.ok(report.blockers.includes('monthlyPlanId:unsupported_ref'))
  assert.ok(report.blockers.includes('stateMachineRef:unsupported_ref'))
