...
async function main() {
  ...
  const dataset = generateMiniHighFidelitySyntheticDataset({
    projectId: 'synthetic-yunhai-coverage',
    taskCount: 600,
    startDate: '2026-04-01',
    months: 42,
    progressSnapshotMode: 'daily',
    durationExperienceSampleCount: 2_000,
  })
  ...
  const result = await runMiniHighFidelitySyntheticStressTest(dataset, {
    ...
    maxCases: 600,
    concurrency: 12,
    asOfDate: '2029-09-20T08:00:00.000Z',
  })
  const artifacts = await writeMiniHighFidelityStressArtifacts(result, {
    fileBase: `high-fidelity-coverage-stress-${new Date().toISOString().slice(0, 10)}`,
  })
