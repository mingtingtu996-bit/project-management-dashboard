...
def test_workflow():
    ...
    with sync_playwright() as p:
        ...
        # 截图
        page.screenshot(path='c:/Users/jjj64/WorkBuddy/20260318232610/artifacts/test-results/01_homepage.png', full_page=True)
        print("  �?首页截图已保�?)
        ...
        print(f"  - 找到 {len(gantt_elements)} 个图表元�?)
        
        # 检查是否有风险相关元素
        risk_elements = page.locator('[class*="risk"], text=风险').all()
        print(f"  - 找到 {len(risk_elements)} 个风险相关元�?)
        
        # 检查里程碑
        milestone_elements = page.locator('[class*="milestone"], text=里程�?).all()
        ...
        for btn in buttons[:5]:
            try:
                if btn.is_visible() and btn.is_enabled():
                    ...
                    page.wait_for_load_state('networkidle')
                    clicked = True
                    print(f"  �?点击了按�? {btn.inner_text()[:20]}")
                    break
            except:
                continue
        ...
        page.screenshot(path='c:/Users/jjj64/WorkBuddy/20260318232610/artifacts/test-results/07_mobile.png', full_page=True)
        print("  �?移动端截图已保存")
        ...
        # 最终截�?
        page.screenshot(path='c:/Users/jjj64/WorkBuddy/20260318232610/artifacts/test-results/08_final.png', full_page=True)
        print("\n📁 所有截图已保存�?test-results 目录")
